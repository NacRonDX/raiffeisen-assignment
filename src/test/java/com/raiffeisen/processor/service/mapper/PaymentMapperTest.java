...
class PaymentMapperTest {
    ...
    void testToDtoWithValidInput() {
        var payment = Payment.builder().id(1L).amount(BigDecimal.valueOf(100.50)).currency("USD").fromAccount("12345")
                .toAccount("67890").timestamp(ZonedDateTime.now()).build();

        var paymentDto = paymentMapper.toDto(payment);

        assertAll(() -> assertEquals(payment.getId(), paymentDto.getId()),
    ...
    void testToEntityWithValidInput() {
        var paymentDto = PaymentDto.builder().id(1L).amount(BigDecimal.valueOf(100.50)).currency("USD")
                .fromAccount("12345").toAccount("67890").timestamp(ZonedDateTime.now()).build();

        var payment = paymentMapper.toEntity(paymentDto);

        assertAll(() -> assertEquals(paymentDto.getId(), payment.getId()),
